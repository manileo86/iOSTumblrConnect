...
#define   kCustomRequestTimedOut  @"Network looks lazy"
...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:kCustomRequestTimedOut,NSLocalizedDescriptionKey,nil]];  
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		NSString *reason = @"Connection Failed!";
