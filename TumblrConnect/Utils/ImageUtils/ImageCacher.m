...
- (void)removeAllImages
{
    ...
    NSString *cacheDirectoryName = [self getCacheDirectoryName];
    NSArray *items = [fileManager contentsOfDirectoryAtPath:cacheDirectoryName error:NULL];
...
- (void)addImageToMemoryCache:(UIImage *)image withKey:(NSString *)key
{
    ...
    if ([keyArray count] > MEMORY_CACHE_SIZE)
    {
        ...
        NSString *lastObjectKey =[[NSString alloc] initWithString:[keyArray lastObject]];
        [memoryCache removeObjectForKey:lastObjectKey];
		[lastObjectKey release];
...
- (NSString *)getCacheDirectoryName
{
    ...
    NSString *cacheDir = [[NSString alloc] initWithString:[paths objectAtIndex:0]];
    NSString *cacheDirectoryName = [[NSString alloc] initWithString:[cacheDir stringByAppendingPathComponent:IMAGE_CACHE_FOLDER_NAME]];
	[paths release];
	[cacheDir release];
	return [cacheDirectoryName autorelease];
}

- (NSString *)getFileNameForKey:(NSString *)key
{
    NSString *cacheDirectoryName = [self getCacheDirectoryName];
    NSString *fileName = [[NSString alloc] initWithString:[cacheDirectoryName stringByAppendingPathComponent:key]];
